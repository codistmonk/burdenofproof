...
template< typename T >
class PeriodicPropertyChronology : public PropertyChronology< T > {
public:
    virtual  T getValue(boost::posix_time::ptime const & time) const override;

    virtual inline ~PeriodicPropertyChronology() override {}

protected:
    std::set< PeriodicTemporalValue < T >,
              PeriodicTemporalValueComparator< T > >    m_values;
};

template< typename T >
class AbsolutePropertyChronology : public PropertyChronology< T > {
public:
    virtual  T getValue(boost::posix_time::ptime const & time) const override;

    virtual inline ~AbsolutePropertyChronology() override {}
