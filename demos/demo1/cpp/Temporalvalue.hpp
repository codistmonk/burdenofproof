...
typedef boost::date_time::date      Date;
...
class TemporalValue {
public:
    explicit TemporalValue(ValueType const & val, TimeType const & time)
        : m_value(new ValueType(val)),
          m_time(time) {}

    inline ValueType const & getValue() const {
        return *m_value;
    }

    inline TimeType const & getTime() const {
        return *m_time;
    }
...
class PeriodicTemporalValue : public TemporalValue< ValueType, Time > {

};

template< typename ValueType >
class AbsoluteTemporalValue : public TemporalValue< ValueType, Time_Duration > {

};

template< typename T>
struct PeriodicTemporalValueComparator {
    inline bool operator()(PeriodicTemporalValue< T > const & v1,
                           PeriodicTemporalValue< T > const & v2) {
        return v1.getTime() < v2.getTime();
    }
};

template< typename T>
struct AbsoluteTemporalValueComparator {
    inline bool operator()(AbsoluteTemporalValue< T > const & v1,
                           AbsoluteTemporalValue< T > const & v2) {
        return v1.getTime() < v2.getTime();
    }
};
