...
namespace burdenofproof {
    ...
    Time const MIN_TIME = Time(boost::posix_time::min_date_time);
    class vec3f;

template< typename ValueType >
class TemporalValue {
 public:
    explicit TemporalValue(ValueType const & val, Time const & time)
    ...
    inline void setTime(Time const & time) {m_time = time;}
...
struct TemporalValueComparator {
    inline bool operator()(TemporalValue< T > const & v1,
                           TemporalValue< T > const & v2) const {
...
//    template< typename T >
//    TemporalValue< T > & TemporalValue< T >::operator=(
//      TemporalValue const & tv) {
//        if (this != &tv) {
//            m_value = tv.m_value;
//            m_time = tv.m_time;
//        }
